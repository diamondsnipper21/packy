...
@section('content')
    ...
    <br /><br />
    @if ($trial === true)
        {{ $memberName }} just started a free trial ({{ $price }}) to join your community <strong>{{ $communityName }}</strong>.
    @else
        {{ $memberName }} just started a subscription ({{ $price }}) to your community <strong>{{ $communityName }}</strong>.
    @endif
    <br /><br />
    Nice work!
    <br /><br />
